...
export class SidebarComponent {
  ...
  getRoleLabel(): string {
    ...
    const map: Record<string, string> = {
      ...
      ROLE_STAFF: 'Staff', ROLE_SUPPLIER: 'Supplier'
    };
    return map[role ?? ''] ?? '';
  }
  ...
  getRoleColor(): string {
    const role = this.auth.getRole();
    const map: Record<string, string> = {
      ROLE_ADMIN: '#ef4444', ROLE_MANAGER: '#6366f1',
      ROLE_STAFF: '#10b981', ROLE_SUPPLIER: '#f59e0b'
    };
