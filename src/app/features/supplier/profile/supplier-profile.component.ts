...
export class SupplierProfileComponent implements OnInit {
  ...
  load(): void {
    this.loading = true;
    this.svc.getMyProfile().subscribe({ next: r => { this.profile = r.data; this.form.patchValue(r.data); this.loading = false; }, error: () => { this.loading = false; } });
  }
  ...
  submit(): void {
    ...
    this.svc.completeProfile(this.form.value).subscribe({
      next: () => { this.toastr.success('Profile updated'); this.editing = false; this.submitLoading = false; this.load(); },
