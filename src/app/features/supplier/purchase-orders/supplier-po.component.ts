...
export class SupplierPoComponent implements OnInit {
  ...
  showDetail = false;
  statuses = ['ALL','SENT','ACCEPTED','SHIPPED','RECEIVED','REJECTED'];
  ...
  load(): void {
    this.loading = true;
    this.svc.getMyPOs().subscribe({ next: r => { this.orders = r.data; this.loading = false; }, error: () => { this.loading = false; } });
  }
  ...
  accept(o: PurchaseOrderResponse): void {
    ...
    ref.result.then(() => { this.svc.acceptPO(o.id).subscribe({ next: () => { this.toastr.success('PO accepted'); this.load(); } }); }).catch(() => {});
  }
  ...
  reject(o: PurchaseOrderResponse): void {
    ...
    ref.result.then((reason: string) => { this.svc.rejectPO(o.id, { reason }).subscribe({ next: () => { this.toastr.success('PO rejected'); this.load(); } }); }).catch(() => {});
  }
  ...
  ship(o: PurchaseOrderResponse): void {
    ...
    ref.result.then(() => { this.svc.shipPO(o.id).subscribe({ next: () => { this.toastr.success('PO marked as shipped'); this.load(); } }); }).catch(() => {});
  }
