...
export class ManagerReportsComponent implements OnInit {
  ...
  trendOptions: ChartConfiguration<'line'>['options'] = { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'top' } }, scales: { x: { grid: { display: false } }, y: { grid: { color: 'rgba(148,163,184,0.1)' } } }, elements: { line: { tension: 0.4 } } };
  ...
  loadStock(): void { this.loading = true; this.svc.managerWarehouseStock().subscribe({ next: r => { this.warehouseStock = r.data; this.loading = false; }, error: () => { this.loading = false; } }); }
  loadLowStock(): void { this.loading = true; this.svc.managerLowStockAlerts().subscribe({ next: r => { this.lowStock = r.data; this.loading = false; }, error: () => { this.loading = false; } }); }
  loadTrend(): void {
    this.loading = true;
    this.svc.managerStockTrend(this.fromDate, this.toDate).subscribe({
      next: r => {
        const t = r.data;
        this.trendData = { labels: t.dailyTrends.map((d: any) => d.date), datasets: [{ label: 'In', data: t.dailyTrends.map((d: any) => d.stockIn), borderColor: '#10b981', backgroundColor: 'rgba(16,185,129,0.1)', fill: true }, { label: 'Out', data: t.dailyTrends.map((d: any) => d.stockOut), borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.1)', fill: true }] };
        this.loading = false;
