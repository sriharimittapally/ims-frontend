...
export class PurchaseOrderService {
  ...
  getMyPOsByStatus(
    ...
    return this.http.get<ApiResponse<PurchaseOrderResponse[]>>(
      `${this.base}/supplier/status/${status}`,
    );
  }

  getMyWarehousePOsByStatus(
    status: string,
  ): Observable<ApiResponse<PurchaseOrderResponse[]>> {
    return this.http.get<ApiResponse<PurchaseOrderResponse[]>>(
      `${this.base}/my-warehouse/status/${status}`,
    );
  }
  ...
  rejectPO(
    id: number,
    request: PurchaseOrderRejectionRequest,
  ): Observable<ApiResponse<PurchaseOrderResponse>> {
    return this.http.put<ApiResponse<PurchaseOrderResponse>>(
      `${this.base}/${id}/reject`,
      request,
